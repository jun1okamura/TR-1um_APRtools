...
> ★ **これは 2026-09-15 の作り直しの記録。** その後 `.lib` を作り直し（U96）、
> `set_load` を `.lib` から引くようにした（U99）ので、**数字は動いている**
> （表の中に現在値を併記した）。手順の正本はフロー文書（`20_`〜`23_`）。
...
| STA（OpenSTA、P&R 前） | 最小周期 **27.512 ns = 36.35 MHz**、hold **+5.381 ns**（2026-09-15 の記録。`.lib` 作り直し後は **27.620 ns = 36.21 MHz**）|

...
| # | 項目 |
|---|---|
| 1 | **チップ組み立て（`assemble_top.py` 以降）は未検証。** フレームが要るので、U19（PDK 版とのずれ）を先に片付ける |
| 2 | ~~`APR_*` への環境変数統一~~ — 手順 2 で完了 |
| 3 | ~~`*_nrow_fm` 剥がし~~ — 手順 3a で完了 |
| 4 | ~~電源ピン名の `vdd`/`vss` 統一~~ — 手順 3b で完了（**コア側のみ**。チップ側は U21 で保留） |
| 5 | `apr/from_sclk_spi/` 9 本の統合 |
| 6 | TD4 / SCLK_SPI でも同じ検証を回す |
| 7 | `<top>.extracted`（`lvs_pnr.py` の出力）は移行前のまま。**LVS は CI で回す**ので実害は無いが、リポジトリの中では古い |
| 8 | `tb_*.spice` の `.include` が絶対パス。設計機で作り直さないと読めない（`docs/31_verify_ngspice.md` §0） |

## 7. 検証環境について

今回は KLayout / gdstk が入る環境で実行した。ngspice の 14 項目は
**設計機（macOS / arm64）でも同じ値で 14/14**（§5-d）。
残りは配置配線で、**設計機で同じ手順を回して同じ md5 になることを一度
確認しておくこと** — ルータの非決定性
（`docs/90_improvement_notes.md` §2-6）は `PYTHONHASHSEED=0` で抑えているが、
根本原因が未特定なので、環境をまたいだ再現性は別途確かめる価値がある。

...
作成: 2026-09-14 / 更新: 2026-09-18 / 状態: **移行は完了**（4 リポジトリとも稼働中）

## docs の読み方 — **「記録」と「参照」を分ける**

| | 何か | 更新するか |
|---|---|---|
| **参照** | いまどうなっているか。`00` / `03` / `04` / `10` / `11` / `20`〜`23` / `30` / `31` / `40` / `50` / `90` / `91` | **する**。事実が変わったら直す |
| **記録** | いつ何が起きたか。`01` / `02` / `05` / `06` / `07` / `08` / `09` / `92` | **しない**。当時の記録なので書き換えない |

★ 記録の先頭には **「これは &lt;日付&gt; の記録。いまの状態は…」**という
断りが入っている。**古い数字が残っていても、それは間違いではなく記録**
（2026-09-18 に、動いた数字には現在値を併記した）。

★ **「いまどうなっているか」は、書くより数えさせる。**
写しの現況は `apr/check_copies.py`、ホームパスは `apr/check_no_home_paths.py`、
台帳の整合は `apr/check_ledger.py`、`.lib` の値は `apr/lib_query.py` が出す。
**文書に写した数字は、いつか古くなる**（U99 で 2 度踏んだ）。

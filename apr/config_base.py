...
STA_MACRO_INSTS = []
# 設計固有の SDC/TCL。`syn/sta/setup.tcl` の直後に連結される。
# **論理的にあり得ないパス**（false path）はここに書く。
# ★ false path は「ここは見なくてよい」という**主張**なので、
#   **根拠を制約の隣にコメントで書く**。1 行の指定では書ききれないから
#   config.py の表ではなくファイルにしてある。
# ★ 指定したファイルが無ければ **STA を止める**。制約が黙って抜けるくらいなら
#   回らない方がよい（U74: 回らなかった検査を「OK」と読まない）。
STA_EXTRA_TCL = None

# 設計の `lef/` に正本（stdcell）の写しがあり、**それを意図して凍結している**とき True。
# 提出時のフロー（例: `TR-1um_I2C_2026/scripts/pnr/`）は写しを入力に読むので、
# 正本を更新しても**揃えてはいけない**（決定 17: 既提出のものは壊さない）。
# 既定は False = 「写しは残っていないのが正常」。
LEF_COPY_FROZEN = False

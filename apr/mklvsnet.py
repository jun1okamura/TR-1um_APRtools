...
出どころ（`lef/simulation/README` の分類でいう **(A) 設計意図**）:
  ...
  **レイアウトからの抽出は一切使っていない**ので、これと抽出ネットリストを突き合わせる
  LVS は同語反復にならない。
...
SIM_DIR = os.path.join(cfg.stdcell_dir(), "simulation")   # APRtools の stdcell へ
PWR = (rules.PWR_NET, rules.GND_NET)
# RTL が電源をポートに持つ設計（I2C は RSLATCH を手挿しするために
# `module i2c_slave_async(..., VDD, GND)` と宣言している）向けの名寄せ。
PWR_ALIAS = {"VDD": rules.PWR_NET, "vdd": rules.PWR_NET, "VCC": rules.PWR_NET,
             "GND": rules.GND_NET, "gnd": rules.GND_NET,
             "VSS": rules.GND_NET, "vss": rules.GND_NET}
...
def main():
    ...
    # 手順 3b でコアのレール名を `vdd` / `vss` に統一したので、RTL が
    # `VDD` / `GND` と宣言していても**正規名に寄せる**。名前を寄せずに足すと
    # 28 本にダブり、レイアウト側（26 本）とポート数が合わずに
    # LVS がグラフマッチに入れない（`docs/40_gotchas.md` §1-1）。
    top_ports = [PWR_ALIAS.get(p, p) for p in top_ports]
    _seen = set()
    ...
    for typ, iname, pins in v["instances"]:
        ...
        ports, _txt = got
        nets = [PWR_ALIAS.get(n, n) for n in
                instance_nets(typ, iname, pins, ports, resolve, dangling)]
        used.add(typ)

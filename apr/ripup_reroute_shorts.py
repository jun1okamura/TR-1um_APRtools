...
（23 ピンのリセットネットを動かして遠くで別の短絡を作った事故がある）。
...
route_channels.py's collision avoidance is spread across 4 drawing
...
project's box/via primitives):
  1. Run the router exactly as-is (no changes to route_channels.py
  ...
  3. For each conflict, rip up ONE side's offending box (the "mover",
     ...
     of the box's original electrical endpoints exactly:
       - VERTICAL (M2) box: a "detour-in-X" jog inserted mid-span (two
         ...
         M1-run pattern route_channels.py's own draw_jog already
...
This never touches route_channels.py's track-claiming bookkeeping
...
Example (v7v2 state, this session):
    python3 ripup_reroute_shorts.py \
        Layout/i2c_slave_async_v7v2_routed.gds \
        script/pin_map_v7v2.json \
        script/net_shapes_v7v2.json \
        LEF/placement_v7_priomch.json \
        98,332,300,280,100 \
        Layout/i2c_slave_async_v7rr_routed.gds \
        script/pin_map_v7rr.json \
        script/net_shapes_v7rr.json
"""
...
def component_conflicts(fixer, unidentified=None):
    ...
    Both ARE seen by step10's `verify_connectivity_m1m2.py`,
...
class Fixer:
    def __init__(self, in_gds, pin_map, net_shapes, ch_y0, ch_heights, row_width, placement=None):
        ...
        arg): the loaded placement_*.json dict (same one used to
    ...
    # ---- live geometry queries (mirrors route_channels.py's
    ...
    def clear_excluding(self, layer_name, x0, y0, x1, y1, exclude_net, margin=0.0):
        ...
        route_channels.py), which is NOT recorded in net_shapes (only the plain

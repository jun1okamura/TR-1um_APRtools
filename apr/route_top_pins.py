...
(Layout/i2c_slave_async_v7rr_routed.gds -- 0 shorts, DRC 0/0/0
...
Track/via/wire conventions are the same ones route_channels.py
...
from highlight_top_pins import (  # noqa: E402
...
IN_GDS = _cfg.artifact("i2c_slave_async_v7rr_routed.gds")
...
def port_to_net_name(port, resolver=None):
    ...
    highlight_top_pins.py's 108.42 note."""
    if "[" in port:
...
def main(placement_json=PLACEMENT_JSON, in_gds=IN_GDS, out_gds=OUT_GDS, ch_heights=CH_HEIGHTS,
    ...
    # ALREADY routed nets (>=2 pins, per route_channels.py) whose
    ...
    # route_channels.py itself writes, design_notes 38.x) and
    ...
    def own_region(net, layer):
        ...
        obstruction (route_channels.py's channel_clear v18)."""
        key = (net, layer)
    ...
    def m2_clear(x0, y0, x1, y1, gap=M2_MIN_GAP, exclude_net=None, extra_own=None):
        ...
        # (as placed by gen_placement.py, NOT something this
        # router itself has drawn). own_region()/net_shapes_log only
        # records wires THIS SCRIPT/route_channels.py drew for a
    ...
    def find_clear_x_vertical(x_start, y_lo, y_hi, half, dogleg_y=None,
        ...
        committing" fix applied throughout route_channels.py
    ...
    # pin-to-trunk connection in route_channels.py already
    ...
    for port, direc, inst, pname, cx, cy in row0_list:
        ...
        # empty (net_shapes_log only tracks nets route_channels.py

#!/usr/bin/env python3
"""gen_cell_spice.py -- build stdcell/<世代>/TR-1um_STDCELL.spice, the project's
...
Override the path with $APR_REF_CIR if the sibling checkout lives elsewhere.
...
apr/check_cell_spice.py expands `m=` before comparing against the GDS.
...
Two differences are expected and reported as such rather than as errors:
  ...
  * FILL2's export declares its pins as `GND VDD`, the .cir as `VDD GND`.  Same
  ...
  usage:  apr/gen_cell_spice.py [--check]
"""
import argparse
import os
import re
...
# **書き出し先も STDCELL 正本**。セルライブラリは設計ではなく APRtools のもの。
OUT_PATH = _cfg.stdcell_file("TR-1um_STDCELL.spice")

DEFAULT_REF_CIR = os.path.join(
    os.path.dirname(_cfg.ROOT), "TR-1um_I2C_2026", "src", "tr_1um_i2c_slave_async.cir"
)
REF_CIR = _cfg.getenv("REF_CIR", DEFAULT_REF_CIR)
...
def _sim_dir():
    ...
    for cand in (_cfg.getenv("XSCHEM_SIM_DIR"),   # lint: ok 外部ツール（xschem）の置き場
                 os.path.join(_cfg.LAYOUT, "step10", "simulation"),
                 _cfg.stdcell_file("simulation"),
...
** standard cells this project places.  GENERATED by apr/gen_cell_spice.py;
** do not edit by hand.
**

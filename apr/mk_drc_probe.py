...
★ 上の 3 つが U18 の答えそのもの。デッキの `M1W = M1.sized(-5.0)…sized(5.0)` は
  ...
  **「10.0 以上が M1(W)」ではなく「10.0 を超えると M1(W)」**。
  2026-09-17 にデッキ（KLayout 0.30.9）で確認: `T_SW_10` 0 件 /
  `T_SW_10p` `M1.SW` 1 件 / `T_SW_TEE` 0 件。

★ **太い方は 10.001 ではなく 10.050 にしてある。** 最初 10.001（1 dbu）で作った
  ところ、デッキは `M1.SW` 1 件に加えて **`ERR01: OFFGRID 0.050` を 6 件**出した。
  `01_Basics.drc` が 12 層すべてに `ongrid(0.050)` を掛けており、**0.001 の刻みは
  そもそも作れない**。この探針は「1 セル 1 規則」が約束なので、**作れる中で
  いちばん細い太線** = 10.050 にした。電源バーの余裕は 1 dbu ではなく
  **製造グリッド 1 つ分（0.050 µm）**である。
...
def build():
    ...
    # ★ 1 dbu（10.001）ではなく製造グリッド 1 つ分。10.001 は ERR01 も出る
    w1 = rules.M1_WIDE_MIN + rules.MFG_GRID  # 10.050 = 作れる中でいちばん細い太線

...
Two different situations, both handled:
  1. A port whose net has exactly ONE pin in the whole design (e.g. each
     ...
     routed by route_channels.py (it only routes nets with >=2
...
modifies the input file):
  ...
  (260, 2): VDD/GND tap-mesh marker boxes at top/bottom margin TAP
            column X positions.
  (260, 3): "vdd"/"vss" text labels for those.
"""
...
# caused route_top_pins.py's gather_pins() (which imports
...
def build_net_pins(placement, ch_heights, row_h):
    """Same construction as route_channels.py's net_pins, kept
...
if __name__ == "__main__":
    PLACEMENT_JSON = _cfg.artifact("placement_v6.json")

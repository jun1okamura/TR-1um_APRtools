...
座標はハードコードせず、配置 GDS のマクロ実体 + LEF のポート + `config`
...
M1_LAYER = (13, 0)
M2_LAYER = (20, 0)
# ★ **1.4 で空き帯を探す。2.0 ではない**（U18、2026-09-17 に実測）。
#   ストラップは幅 10.000 ちょうどで、M1(W) の判定境界の内側 1 dbu にいるので
#   M1.SW（2.0）は**かからない**。ただし余裕は製造グリッド 1 つ分（0.050 µm）だけ:
#   TD4 の提出 GDS では、このストラップの 1.8 µm 上を幅 3.4 の M1 レールが
#   走っている箇所が 2 つあり、**幅を 1 つでも増やすと M1.SW が 2 件出る**。
#   幅を触るなら、ここを 2.0 にするのが先。
#   （`POWER_BAR_GAP` は**ストラップ 2 本の間**の隙間で、周りとの間隔ではない）
...
def main():
    ...
    ap.add_argument("--width", type=float, default=cfg.POWER_BAR_W,
                    help="ストラップの幅 µm（既定 = config.POWER_BAR_W）")

...
Stages, each leaving its own GDS:
  ...
  step6  channel routing (5 internal passes,  route_channels.py
         each checkpointed as its own GDS)
  step7  short rip-up / re-route              ripup_reroute_shorts.py
  step8  top-level pin pull-out               route_top_pins.py
  step9  VDD/GND chip-level pins              add_power_pins.py
  step10 channel compaction                   squeeze_channels.py
...
def _macro_nets():
    ...
    try:
        ...
        return set()
    # マクロは row0 とは限らない（縦置きの `APR_MACRO_ROW`）。全行を見る。
    for row in pl["rows"]:
        for i in row:
...
def per_row_local_nets():
    ...
    #
    # `APR_PRL_NETS`（','区切り）で**名指しの追加**ができる。しきい値を下げる
...
POWER_PORTS = {rules.PWR_NET, rules.GND_NET, "VDD", "VSS", "GND"}  # 旧名も弾く
...
def check_port_pins(gds, extra=(rules.PWR_NET, rules.GND_NET)):
...
def checks(gds, pin_map, ch, squeezed=False):
    print("\n=== DRC ===")
    subprocess.run([sys.executable, os.path.join(HERE, "drc_check.py"),
    ...
    subprocess.run([sys.executable,
                    os.path.join(HERE, "verify_connectivity_m1m2.py"),
...
def stage11(ch_heights):
    ...
    **圧縮（step10）の後**であること。前に入れると y が動く/削られる。
    ...
    if not getattr(cfg, "MACRO_POWER", False):
        print("  APR_MACRO_POWER=0 -- マクロ電源の接続はしない")
        return

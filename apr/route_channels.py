...
IN_GDS = _cfg.artifact("i2c_slave_async_placement.gds")
OUT_GDS = _cfg.artifact("i2c_slave_async_routed.gds")
...
# must match gen_placement_gds.py
...
def main(placement_json=PLACEMENT_JSON, in_gds=IN_GDS, out_gds=OUT_GDS,
    ...
    if side_bus_nets and len(side_bus_nets) > len(_bus_x):
        raise SystemExit(f"縦バスに乗せるネットが {len(side_bus_nets)} 本、"
                         f"列は {len(_bus_x)} 本しかない（APR_SIDE_BUS を増やす）")
    ...
    if pin_map_path is None:
        pin_map_path = _cfg.PIN_MAP_JSON
    if net_shapes_path is None:
        net_shapes_path = _cfg.NET_SHAPES_JSON
    ...
    # FILLPRI_* reserved-FILL2 instances gen_placement.py places
    ...
    # row packing (gen_placement.py's case-2 mitigation) alone
    ...
    for net, pads in net_pins.items():
        ...
        if net in down_nets:
            ...
            # 横倒し（帯）では常に 0、縦置きでは `APR_MACRO_ROW`。
    ...
    guard_idx = defaultdict(set)  # (section 44) channel -> {idx, ...} that MUST
                                   ...
                                   # compaction pass (squeeze_channels.py)
    ...
    # ground truth for gen_err_report.py's exact short-overlap
    ...
    def find_channel_clear_x(start_x, y_lo, y_hi, extra_ok=None):
        ...
        # verify_connectivity.py / fix_v9_remaining_shorts.py-
    ...
    per_row_spine_events_path = _cfg.PER_ROW_SPINE_JSON
    ...
    if force_jog_events_path is None:
        force_jog_events_path = _cfg.FORCE_JOG_EVENTS_JSON
    ...
    if compaction_info_path:
        ...
        # geometric squeeze (squeeze_channels.py) needs to safely

...
power-mesh straps (route_channels.py's draw_tap_power_mesh
...
route_top_pins.py's add_m2_pin()/add_m1_pin(), which drops a
...
them signal port names, zero "vdd"/"vss". (highlight_top_pins.py's
...
placement_v9.json, identical across all rows by construction)
and for both nets (GND at local offset (1.0,4.4), VDD at (6.4,9.8) --
route_channels.py's TAP_GND_X_LOCAL/TAP_VDD_X_LOCAL), drop one
M2PIN+TXM2 "vss"/"vdd" pin marker at the bottom BBOX edge (Y=0) and
one at the top BBOX edge (Y=core_h), using the exact same PIN_SIZE_UM
box-centered-on-label convention route_top_pins.py's
...
PIN_SIZE_UM = 3.0  # matches route_top_pins.py's PIN_SIZE_UM

PLACEMENT_JSON = _cfg.PLACEMENT_JSON
IN_GDS = _cfg.artifact("v9_step_8_squeezed_top_pins_routed.gds")
OUT_GDS = _cfg.artifact("v9_step_9_power_pins_added.gds")
...
def main(placement_json=PLACEMENT_JSON, in_gds=IN_GDS, out_gds=OUT_GDS, core_h=None):
    ...
    n = 0
    for tap_x in tap_xs:
        for label, (lx0, lx1) in ((rules.GND_NET, TAP_GND_X_LOCAL), (rules.PWR_NET, TAP_VDD_X_LOCAL)):
            cx = tap_x + (lx0 + lx1) / 2.0
            # bottom BBOX edge (Y=0): box spans [0, PIN_SIZE_UM], same
            # convention as route_top_pins.py's row0 signal pins

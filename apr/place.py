...
TD4 で増えたもの = **ハードマクロ `REG8x16`**。
  ...
  `config.macro_box()` はマクロを ch[0] ぶん持ち上げて **row0 の底面と
...
# 横倒し（帯）は row0 の下なので -1。縦置きで `APR_MACRO_ROW=k` なら k-1。
...
# 標準セルだけなので、`config.MACRO_MODE == "none"` のときはマクロの段を
...
#
# ポートの出る辺は `route_top_pins.py` の規約で**行から決まる**:
...
def order_rows(assign, width, net_cells, ports, rows_y, n, mpin,
    ...
    # なる縦置き `APR_MACRO_ROW>=1` では、これを外さないと hpwl が
    # `KeyError: 'u_mem'` で落ちる。
...
def fixed_blocks(row_w):
    ...
    優先的に使う（`route_channels.py` が `FILLPRI_*` を自動検出する）。
    """
    ...
    # 狙い撃ちの追加コリドー（`APR_PRI_X`）。既存の固定ブロックと重なる物と
...
def main(net_path=None, info_path=None, restarts=800, order_passes=40,
    ...
    if tight:
        print(f"  ** ch{tight} は見積りが予算を超えている（ルータのジョグでさらに"
              f"増えるので config.CH_HEIGHTS を見直すこと）")

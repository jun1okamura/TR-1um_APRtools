...
"""mkchipnet.py -- チップレベルの LVS ソースネットリスト。
    ...
    layout/portrait/simulation/td4_soc_arr.spice  （コア、単体で LVS 済み）
...
# コアのポートは rules.PWR_NET / GND_NET（小文字）。チップ側のレール名は
# フレームに合わせて大文字（docs/04_naming.md §1-5、U21）。ここで写像する。
RAIL = {"VDD": "VDD", "GND": "VSS", "VSS": "VSS",
        rules.PWR_NET: "VDD", rules.GND_NET: "VSS"}

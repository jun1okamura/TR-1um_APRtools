...
Method:
  1. Run route_channels.py once (or reuse an existing run's
  ...
  3. Re-run gen_placement_gds.py and route_channels.py
     with the new CH_HEIGHTS, then drc_check.py and
     verify_connectivity.py to confirm the compression didn't
...
Run standalone (compresses the ORIGINAL netlist's pipeline by default):
    python3 script/compress_channels.py
...
import route_channels as R  # noqa: E402
import gen_placement_gds as G  # noqa: E402
...
def main(placement_json=R.PLACEMENT_JSON, in_gds=R.IN_GDS,
    ...
    if out_prefix is None:
        out_prefix = _cfg.artifact("i2c_slave_async_compressed")
    ...
    print(f"\nwrote {routed_gds}")
    print("Run drc_check.py / verify_connectivity.py on this file to confirm "

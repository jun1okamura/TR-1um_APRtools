...
Yosys 用 genlib と area_estimate.py 用の面積テーブルを生成する。
  ...
  usage: python3 $APRTOOLS/apr/cellinfo.py lef/TR-1um_STDCELL.gds [--genlib out.genlib]
                                     [--areas scripts/cell_area.json] [--check]

`--areas` は全セルの実測面積を JSON に書き出す。`$APRTOOLS/apr/area_estimate.py` が
...
def genlib(rows):
    ...
    out = [f"# TR-1um STDCELL - area from GDS (235,0) abutment box [um^2], row height {ROW_H}um",
           "# generated by $APRTOOLS/apr/cellinfo.py   (delays are placeholders)",

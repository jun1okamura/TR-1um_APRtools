...
なぜ要るか: もともとは**セルの GDS を直したのに、そこから作る
`TR-1um_PNR.gds` が古いまま**だと、フローが古いセルで回って
**落ちずに古いチップが出る**からだった（U77）。**U89 で正本を 1 本にした**
ので、いまは `selfcheck.py` 3b が「同じファイルか」を見るだけで済む。
読み手はここに残してある（設計が `CELL_GDS` を上書きしたときと、
`reference/` の凍結コピーを突き合わせるときに要る）。
"""

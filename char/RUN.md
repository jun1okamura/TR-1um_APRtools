...
★ **`TR1UM_CELLDIR` の既定も `cells_gds` です**（2026-09-18 に変えました。
それまでは `cells_ext` が既定で、環境変数を付けずに回すと**黙って古い方を
読んで**いました）。無ければ `cells_ext` → `cells` の順に落ちます。
いま何を読んでいるかは各道具が見出しに出します。

...
**解決（2026-09-16）: 抽出ネットリストの不具合ではなかった。**
**`TLAT` の書込みが、記憶ノードの接合容量に頼って成立していた。**

  症状: `lef/extracted/REG8x16.extracted` を回すと**どのアドレスを読んでも 5V**。
        設計ネットリスト（`lef/simulation/REG8x16.spice`）なら期待どおり。

  ★ 段ごとに測って、こう絞れた（`--probe`）:
      デコーダ      正常（`WR` は 16 本中 5 本、書込みの瞬間に 1 本だけ）
      入力バッファ  正常（セル側の `D` 8 本が 0V/5V に振れる）
      デコードの一致 正常（書く行と読む行は同じ）
      書込みそのもの 正常（**書込み中はセル内 4 ノードが全部正しく反転**）
      → **`WR` が下りた瞬間に、丸ごと元の値へ戻る**

  ★ 決め手（`--strip`）:
      抽出 + 接合を PDK 既定へ（`--strip both`）      **保持する**
      設計 + 接合を 0 へ      （`--strip zero-both`）  **保持しない**
  ...
  ★ **PDK の既定式は「孤立した拡散」を仮定している**
    ...
    実レイアウトの `TLAT` は直列段が**拡散を共有**しているので、抽出は
    共有ぶんを分けて出す。例（NMOS W=3.4u、既定 AS=9.52p / PS=12.4u）:

        XM$10 … AS=5.1p AD=5.1p PS=6.4u PD=6.4u
        5.1p / 3.4u = 1.5u       （2.8u の拡散を 2 素子で分けた形）
        6.4u = 3.4 + 2 x 1.5     （共有した辺を周長から外した形）

    **抽出の方が物理的に正しく、既定式が過大**。全素子で既定の 0.21〜1.00 倍。
    だから**設計ネットリストでは余裕があるように見え、実物の値で落ちる**。
    「抽出が壊れている」ではなく「**設計ネットリストが甘い**」が正しい。

  ★ ただし**どちらも真実ではない**。この抽出は配線容量を持たない
    （`AS/AD/PS/PD` だけが実測）。実レイアウトの記憶ノードには金属・
    ポリの容量も付くので、**本当の余裕は 2 つの中間**にある。
    数字で言うには**寄生容量まで入れた抽出**が要る。

  ★ 電荷注入で飛ぶこと自体は素直な話で、**拡散容量が無ければ
    ゲートと S/D のカップリングが支配する**（設計者コメント、2026-09-16）。
    測って意味があったのは「消したら飛ぶ」ではなく、
    **実レイアウトの実測値でも飛ぶ**という方。

  ★ 以前ここに書いていた「原因は `PS/PD`。`PS/PD` だけ外すと動く
    （22.28ns -> 21.81ns）」は**再現しない**。抽出で 5 通り試して、
    動いたのは `--strip both`（AS/AD と PS/PD を**両方**既定へ）だけ:

        そのまま / perim / area / zero-both   保持しない
        both                                  保持する

    **片方だけでは足りない**ので、「`PS/PD` だけ」という切り分け自体が
    成り立っていなかった。

  ★ **実チップが動かないという話ではない。** TD4 のチップ ngspice
    （12 サイクル）は**抽出ネットリストで通っている**。この測定の刺激は
    `WEB` の縁が 0.1ns と鋭く、injection が最悪になる条件。
    残っているのは「**どこまで鋭い縁なら壊れるか**」の定量化。

  次にやるなら: `EDGE` を振って壊れる縁を求める / `TLAT` に弱い
  キーパーを足す検討（面積は増える）/ 書込みパスの特性化（U7）は
  この余裕の話を織り込んでから。
"""

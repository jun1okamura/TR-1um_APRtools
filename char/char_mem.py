...
**書込みパスの実測値（U7）** — 抽出ネットリスト / typ 5V 25C /
**配線容量なし**（`AS/AD/PS/PD` だけが実測。2026-09-16 の設計者判断）:

  ① アドレス保持  `WEB↑` -> アドレス変更   **6.0 ns**（5.5 ns 不可）
  ② `WEB` 最小低幅                          **11 ns**（10.5 ns 不可）
  ③ データ保持    `WEB↑` -> データ変更     **6.0 ns**（5.5 ns 不可）
  参考  `WEB↓` -> 内部 `WR` 立上り 11.8 ns / `WEB↑` -> 立下り 8.2 ns
        境界（低 12ns）では内部 `WR` は幅 4.8ns・ピーク 4.10V で**書ける**

  ★ **数字を外に出すときは必ず「配線容量なし」の条件を併記する。**
    外れる向き:
      読出し  楽観側（配線容量が乗れば遅くなる。いま `.lib` に入って
              いるのは設計ネットリストの値で、抽出より約 12% 悲観）
      書込み  楽観側（行線 `WR`/`WRB`/`RD`/`RDB` は 8 セル幅、列線
              `D`/`Q` は 16 セル高。縁がなまれば内部 `WR` は細く遅く
              なるので、最小 `WEB` 低幅も保持時間も**実測より大きく**なる）
    余裕は使う側で積む。

  ★ ①と③は 0.5ns 刻みで**同じ 6.0 ns**。同じ仕掛け（どちらも**書込みが確定するまで**
    動かせない）。内部 `WR` が落ちるのは 8.2 ns なので、**2ns ほどの猶予**
    （変えても崩しきれない時間）がある。
"""
